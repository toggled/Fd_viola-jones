...
def learn(positives, negatives, T):
    ...
    for f in FeatureTypes:
        for width in range(f[0],window,f[0]):
            for height in range(f[1],window,f[1]):
    ...
    for i in range(T):
        ...
        for feature, feature_votes in votes.iteritems():
            ...
            #for each feature set its threshold
                #find threshold
            for intimg,featurevalue in sorted(feature.score.items(),key = lambda x: x[1]):
                print intimg.weight,'-> ',intimg.label,' -> ',featurevalue
        ...
        if best_error != 0:
            ...
            classifiers.append((feature, 10000))
            break
